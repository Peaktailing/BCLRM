...
def validate_inventory_inputs(
    ...
    Returns:
        (是否校验通过, 错误信息)
    """
    result = inventory_service.validate_inventory_inputs(
        reagent_name, cas_number, remaining_quantity, specification
    )
    return result.success, result.message


def verify_controlled_status(cas: str) -> Tuple[bool, str]:
    """检查化学品是否为管控类型（向后兼容）
    ...
    Returns:
        (是否管控, 管控类型描述)
    """
    result = inventory_service.verify_controlled_status(cas)
    if result.is_success():
        return result.data
    return False, ""
...
def process_batch_inventory(items: List[dict]) -> Tuple[int, int, List[dict]]:
    """批量处理试剂入库（向后兼容）
    ...
    Returns:
        (成功数量, 失败数量, 失败详情列表)
    """
    result = inventory_service.process_batch_inventory(items)
    if result.is_success():
        data = result.data
        return data["success_count"], data["fail_count"], data["failures"]
    return 0, len(items), [{"row": i + 1, "reagent_name": item.get("reagent_name", "未知"), "error": result.message}
                           for i, item in enumerate(items)]


def retrieve_inventory_by_barcode(barcode: str) -> Optional[ReagentBottle]:
    """根据条码查询入库记录（向后兼容）
    ...
    """
    result = inventory_service.retrieve_inventory_by_barcode(barcode)
    if result.is_success():
        return result.data
    return None


def retrieve_all_inventory() -> List[ReagentBottle]:
    """获取所有试剂瓶库存记录（向后兼容）

    Returns:
        ReagentBottle对象列表
    """
    result = inventory_service.retrieve_all_inventory()
    if result.is_success():
        return result.data
    return []


def filter_inventory(keyword: str) -> List[ReagentBottle]:
    """搜索试剂瓶库存（向后兼容）
    ...
    Returns:
        匹配的ReagentBottle对象列表
    """
    result = inventory_service.filter_inventory(keyword)
    if result.is_success():
        return result.data
    return []


def get_available_chemical_names() -> List[str]:
    """获取可用的化学品名称列表（向后兼容）

    Returns:
        化学品名称字符串列表
    """
    result = inventory_service.get_available_chemical_names()
    if result.is_success():
        return result.data
    return []
...
def get_available_suppliers() -> List[str]:
    ...
    Returns:
        供应商名称字符串列表
    """
    result = inventory_service.get_available_suppliers()
    if result.is_success():
        return result.data
    return []


def get_available_storage_locations() -> List[str]:
    """获取可用的存储位置列表（向后兼容）

    Returns:
        存储位置名称字符串列表
    """
    result = inventory_service.get_available_storage_locations()
    if result.is_success():
        return result.data
    return []


def get_available_reagent_types() -> List[str]:
    """获取可用的试剂类型列表（向后兼容）

    Returns:
        试剂类型名称字符串列表
    """
    result = inventory_service.get_available_reagent_types()
    if result.is_success():
        return result.data
    return []


def get_chemical_info_by_name(name: str) -> Optional[dict]:
    """根据名称获取化学品详细信息（向后兼容）
    ...
    Returns:
        化学品信息字典（包含cas等字段）或None
    """
    result = inventory_service.get_chemical_info_by_name(name)
    if result.is_success():
        return result.data
    return None
...
# 向后兼容的函数别名（原函数名）
reagent_inventory = create_inventory_record
batch_inventory = process_batch_inventory
get_inventory_by_bottle_number = retrieve_inventory_by_barcode
