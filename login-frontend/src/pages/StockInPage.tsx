...
import { PackageOpen, Info, CheckCircle, ClipboardList } from "lucide-react";
import { PURCHASE_ORDERS, type PurchaseOrder } from "@/data/mock";
...
export default function StockInPage() {
  ...
  const [poImported, setPoImported] = useState(false);
  ...
  const handlePoImport = () => {
    ...
    setPoImported(true);
    setTimeout(() => setPoImported(false), 4000);
  };

  const purchasingOrders = PURCHASE_ORDERS.filter((po) => po.status === "采购中");
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {[
          { key: "single" as TabKey, label: "单瓶入库" },
          { key: "purchase" as TabKey, label: "按采购单入库" },
        ].map((tab) => (
          <button
            key={tab.key}
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  {formData.quantity > 1 && (
                    <p className="mt-1 text-xs text-accent-blue">
                      将批量入库 {formData.quantity} 瓶，自动分配连续瓶号
                    </p>
                  )}
                </div>
                <div>
              ...
              {submitted && (
                ...
                >
                  <CheckCircle size={16} />
                  成功入库 {formData.quantity} 瓶，瓶号范围：{10001}-{10001 + formData.quantity - 1}
                </motion.div>
              )}
            </form>
          </motion.div>
          ...
          >
            ...
            <div className="space-y-3 text-sm text-text-sub">
              {[
                ...
                { title: "批量入库", desc: "入库数量大于1时将自动生成连续瓶号，同一批次试剂共享供应商和存储位置。" },
        ...
        >
          ...
          {purchasingOrders.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-text-muted">
              <PackageOpen className="mb-3 h-10 w-10 opacity-30" />
              <p className="text-sm">暂无"采购中"状态的采购单</p>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {purchasingOrders.map((po) => (
                  <button
                    key={po.id}
                  ...
                  >
                    <div className="font-medium text-text-main">{po.po_number}</div>
                    <div className="mt-1 text-xs text-text-sub">{po.semester}</div>
                    <div className="mt-2 text-sm text-text-sub">
                      {po.items.length} 项待入库
                    </div>
                  </button>
                ))}
              </div>
              ...
              {selectedPO && (
                <div className="mt-4 border-t border-border pt-4">
                  <h3 className="mb-3 text-sm font-semibold text-text-main">
                    采购单 {selectedPO.po_number} 明细
                  </h3>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr>
                          <th className="text-left whitespace-nowrap px-3 py-2 font-medium text-xs text-text-muted border-b border-border">试剂名称</th>
                          <th className="text-left whitespace-nowrap px-3 py-2 font-medium text-xs text-text-muted border-b border-border">规格</th>
                          <th className="text-left whitespace-nowrap px-3 py-2 font-medium text-xs text-text-muted border-b border-border">采购数量</th>
                          <th className="text-left whitespace-nowrap px-3 py-2 font-medium text-xs text-text-muted border-b border-border">单位</th>
                          <th className="text-left whitespace-nowrap px-3 py-2 font-medium text-xs text-text-muted border-b border-border">供应商</th>
                        </tr>
                      </thead>
                      <tbody>
                        {selectedPO.items.map((item) => (
                          <tr key={item.id}>
                            <td className="whitespace-nowrap px-3 py-2.5 text-text-main">{item.reagent_name}</td>
                            <td className="whitespace-nowrap px-3 py-2.5 text-text-sub">{item.spec}</td>
                            <td className="whitespace-nowrap px-3 py-2.5 font-medium text-text-main">{item.purchase_qty}</td>
                            <td className="whitespace-nowrap px-3 py-2.5 text-text-sub">{item.unit}</td>
                            <td className="whitespace-nowrap px-3 py-2.5 text-text-sub">{item.supplier}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <button
                    ...
                    className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-white shadow-primary transition-all hover:bg-primary-dark hover:shadow-primary-hover"
                  >
                    <PackageOpen size={16} />
                    确认批量入库
                  </button>
                  {poImported && (
                    <motion.div
                      initial={{ opacity: 0, y: -8 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="mt-3 flex items-center gap-2 rounded-lg bg-accent-green/10 px-4 py-3 text-sm text-accent-green"
                    >
                      <CheckCircle size={16} />
                      成功批量入库 {selectedPO.items.reduce((sum, i) => sum + i.purchase_qty, 0)} 瓶
                    </motion.div>
                  )}
                </div>
              )}
