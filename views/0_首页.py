...
with col1:
    with st.container(border=True):
        ...
        st.page_link("views/1_实时库存.py", label="进入", icon="📦", use_container_width=True)

with col2:
    with st.container(border=True):
        st.markdown("### ➕ 试剂入库")
        st.markdown("将新试剂录入系统")
        st.page_link("views/2_试剂入库.py", label="进入", icon="➕", use_container_width=True)

with col3:
    with st.container(border=True):
        st.markdown("### 📤 领用归还")
        st.markdown("试剂领用和归还操作")
        st.page_link("views/3_领用归还.py", label="进入", icon="📤", use_container_width=True)
...
with col4:
    with st.container(border=True):
        ...
        st.page_link("views/4_综合查询.py", label="进入", icon="🔍", use_container_width=True)

with col5:
    with st.container(border=True):
        st.markdown("### 📊 数据看板")
        st.markdown("统计报表和可视化图表")
        st.page_link("views/5_数据看板.py", label="进入", icon="📊", use_container_width=True)

with col6:
    with st.container(border=True):
        st.markdown("### 🧪 化学品信息")
        st.markdown("管理化学品基础信息")
        st.page_link("views/7_化学品信息管理.py", label="进入", icon="🧪", use_container_width=True)
...
with col7:
    with st.container(border=True):
        ...
        st.page_link("views/8_管控化学品目录.py", label="进入", icon="📋", use_container_width=True)
...
if st.session_state.get("user_role") == "super_admin":
    with col8:
        with st.container(border=True):
            st.markdown("### ⚙️ 系统设置")
            st.markdown("用户管理与系统配置")
            st.page_link("views/6_系统设置.py", label="进入", icon="⚙️", use_container_width=True)
